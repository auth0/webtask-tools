...
module.exports = function (options, ctx, req, res, routingInfo) {
    ...
    // if a 'state' query parameter is present, verify cookie presence and decode the value
    var stateInfo = {};
    if (ctx.query && ctx.query.state) {
        if (req.headers.cookie) {
            var cookies = cookie.parse(req.headers.cookie);
            if (cookies[ctx.query.state]) {
                stateInfo = JSON.parse(base64url.decode(cookies[ctx.query.state]));
                res.setHeader('Set-Cookie',
                    cookie.serialize(ctx.query.state, '', { expires: new Date(0) }));
            }
        }
    }
    ...
    function issueAccessToken(id_token, access_token) {
        ...
        return options.loginSuccess(ctx, req, res, routingInfo.baseUrl, stateInfo);
    }
};
