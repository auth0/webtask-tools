...
module.exports = function (webtask, options) {
    ...
    if (options.getReturnState && typeof options.getReturnState !== 'function') {
        throw new Error('The getReturnState option, if specified, must be a function that accepts (ctx, req) and returns an object to be returned upon a login attempt.');
    }
    ...
    options.getReturnState = options.getReturnState || function (ctx, req, routingInfo, stateName) {
        // Return the state object for post-login processing in loginSuccess(), or false-y to skip any state processing
        // Example: return { target: routingInfo.baseUrl + routingInfo.appPath, query: routingInfo.query };
        return false;
    };
    options.loginSuccess = options.loginSuccess || function (ctx, req, res, baseUrl, stateInfo) {
        // stateInfo is an empty object if getReturnState() returns false-y
        var query = Object.assign({}, stateInfo.query || {}, { access_token: ctx.accessToken });
        res.writeHead(302, { Location: (stateInfo.target || baseUrl) + '?' + require('querystring').stringify(query) });
        return res.end();
    };
...
function createAuthenticatedWebtask(webtask, options) {
    ...
    return function (ctx, req, res) {
        ...
        case '/login':
            handleLogin(options, ctx, req, res, routingInfo); break;
        case '/callback':
            handleCallback(options, ctx, req, res, routingInfo); break;
        default:
            handleAppEndpoint(webtask, options, ctx, req, res, routingInfo); break;
        }
        return;
    };
}
